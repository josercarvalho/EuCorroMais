...
namespace EuCorro.MVC.Site.Areas.Admin.Controllers
{
    ...
    public class EventoController : Controller
    {
        ...
        public ActionResult Edit(Evento evento, IEnumerable<HttpPostedFileBase> file)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    arquivoDel = Path.Combine(Server.MapPath("~/Uploads/Banners"), (string)Session["Kit"]);
                    if (System.IO.File.Exists(@arquivoDel))
                    {
                        try
                        {
                    ...
                    arquivoDel = Path.Combine(Server.MapPath("~/Uploads/Banners"), (string)Session["Patrocinio"]);
                    if (System.IO.File.Exists(@arquivoDel))
                    {
                        try
                        {
        ...
        #region Views Modalidade Evento

        // GET: Admin/Evento/Modalidades/5
        public ActionResult Modalidades(int id)
        {
        ...
        public JsonResult GetEventoById()
        {
            ...
            var profile = _evento.ListarModalidadeEvento(id);
        ...
        /// <returns></returns>
        [HttpPost]
        public HttpStatusCodeResult SaveModalidade(ModalidadeEvento modalidade, ModalidadePreco[] modalidadePrecos)
        {
            try
            {
                ...
                foreach (var item in modalidadePrecos)
                {
                    ...
                    {
                        ModalidadeEventoId = modalidade.ModalidadeEventoId,
        ...
        public ActionResult AddCategoria()
        {
            ...
            ViewBag.Categoria = _evento.ListarCategorias().Select(x => new { x.CategoriaId, x.Nome }).ToList();

            return PartialView("_Categoria", obj);
        }
